...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    'Welcome to Crypto Data Bot! Use /register to get started.'
  )
})
...
bot.onText(/\/register/, async (msg) => {
  ...
  bot.sendMessage(
    userId,
    `Welcome to Crypto Data Bot, ${username}! Use /help to see the list of available commands.`
  )
})
...
bot.onText(/\/get_token (.+)/, async (msg, match) => {
  ...
  if (data) {
    ...
    bot.sendMessage(
      chatId,
      'Failed to fetch data. Please check the token symbol and try again.'
    )
  }
})
  ...
  try {
    ...
    if (alerts.length === 0) {
      ...
      alerts.forEach((alert, index) => {
        response += `${index + 1}. [${alert.id
          }] ${alert.tokenSymbol.toUpperCase()} - ${alert.condition} $${alert.priceThreshold
          }\n`
      })
      bot.sendMessage(chatId, response)
    }
    ...
    for (const alert of alerts) {
      ...
      if (data) {
        ...
        if (alertTriggered) {
          bot.sendMessage(
            alert.userId,
            `Alert triggered: ${alert.tokenSymbol.toUpperCase()} is ${alert.condition
            } $${alert.priceThreshold}. Current price: $${currentPrice}`
          )
        }
      }
    }
